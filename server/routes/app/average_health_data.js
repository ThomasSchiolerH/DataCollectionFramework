...
//         let moodAnalysis = Array.from({ length: highestValue - lowestValue + 1 }, (_, index) => ({
...
//             [inputType || 'mood']: item[inputType || 'mood'],
...
avgHealthRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Filter user input data for the specified type (e.g., "happiness")
      ...
      moodAnalysis = moodAnalysis.map((item, index) => ({
        inputType: inputType || "mood", // Explicitly include inputType
        moodValue: lowestValue + index, // Explicitly calculate moodValue
        avgSteps: item.countSteps ? item.avgSteps / item.countSteps : 0,
        avgExerciseTime: item.countExerciseTime ? item.avgExerciseTime / item.countExerciseTime : 0,
        avgHeartRate: item.countHeartRate ? item.avgHeartRate / item.countHeartRate : 0,
        avgBMI: item.countBMI ? item.avgBMI / item.countBMI : 0,
